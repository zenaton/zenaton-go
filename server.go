...
func (c *ServeConfig) Protocol() plugin.Protocol {
	result := plugin.ProtocolNetRPC
	if c.GRPCServer != nil {
		result = plugin.ProtocolGRPC
	}
...
func Serve(opts *ServeConfig) {
	...
	var server plugin.ServerProtocol
	switch opts.Protocol() {
	case plugin.ProtocolNetRPC:
...
func serverListener_unix() (net.Listener, error) {
	tf, err := ioutil.TempFile("", "z-plugin")
	if err != nil {
		return nil, err
	}
	path := tf.Name()
